...
BATCH_SIZE = 64;
LAST_BUMP_EP = Ref(0)
THR_LADDER = [ 0.930,0.940,0.950,0.970, 0.980, 0.982, 0.983, 0.984, 0.985, 0.986, 0.987, 0.988, 0.989, 0.990, 0.991, 0.992, 0.993, 0.994, 0.995, 0.996, 0.997, 0.998, 0.999, 0.9991,0.9992, 0.9993, 0.9995,0.9999];
THR_IDX      = Ref(1) ;
SUCCESS_THR  = Ref(THR_LADDER[THR_IDX[]]);
N_UPDATE_EPOCHS = 4;
GAMMA = 0.99 ;
LAMBDA = 0.95;
CLIP_RANGE = 0.05 #provare 0.3 forse??;
...
function main_training_loop_parallel(envs::Vector{<:QuantumEnv}, agent::PPOAgent, num_episodes::Int)
    ...
    for episode in 1:num_episodes
        ...
        max_fidelity, idx = findmax(fidelities)
        best_fid_global = max(best_fid_global, max_fidelity)
