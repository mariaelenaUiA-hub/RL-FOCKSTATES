...
function maybe_bump_threshold!(episode_fidelities::Vector{Float64},
                               ...
                               q_level::Float64=0.3,
                               q_factor::Float64=0.75,
                               hysteresis_margin::Float64=1e-4,
                               cooldown::Int=5,                # cooldown per bump↑
                               episode::Int=0,
                               debug::Bool=true,
                               reset_on_bump::Bool=false,
                               # --- NUOVI PARAMETRI (rollback) ---
                               allow_down_bump::Bool=true,
                               down_cooldown::Int=10,          # cooldown per bump↓
                               rollback_drop::Float64=0.10,    # soglia calo mediana10 per rollback
                               min_gap_for_down::Float64=0.02  # quanto la mediana deve stare sotto la soglia corrente
                               )
    ...
    if W_eff < max(5, fld(W, 2)); return nothing; end

    w_sr = sr_hist[end-W_eff+1:end]
    w_f  = episode_fidelities[end-W_eff+1:end]

    sr_win = Statistics.mean(w_sr)
    medF   = Statistics.median(w_f)
    qf     = Statistics.quantile(w_f, q_level)

    curr_thr = SUCCESS_THR[]

    # target dinamico per SR
    dyn_target = sr_target === nothing ?
                 (curr_thr < 0.60 ? 0.70 : (curr_thr < 0.80 ? 0.80 : 0.85)) :
                 sr_target

    # streak ultimi K
    sr_recent  = sr_hist[end-K_eff+1:end]
    sr_streak  = Statistics.mean(sr_recent .>= dyn_target)

    # gate verso la prossima soglia
    next_idx = min(THR_IDX[] + 1, length(THR_LADDER))
    next_thr = THR_LADDER[next_idx]
    q_gate   = q_factor * next_thr

    # ---- BUMP ↑ (come prima) ----
    do_bump_up = ((sr_win ≥ dyn_target) && (sr_streak ≥ 0.60) && (qf ≥ q_gate)) ||
                 (best_fid ≥ next_thr)
    ...
    if do_bump_up
        ...
        # aggiorna entropia (come tuo codice)
        thr = SUCCESS_THR[]
        entropy_floor = if     thr < 0.85;   0.02
                         elseif thr < 0.99;   0.005
                         elseif thr < 0.995;  0.001
                         else                 1e-4
                         end
        decay = (thr < 0.99) ? 0.997 : 0.98
        ew = agent.entropy_loss_weight
        ew = isfinite(ew) ? ew * decay : entropy_floor
        agent.entropy_loss_weight = max(ew, entropy_floor)

        LAST_BUMP_EP[] = (episode == 0 ? LAST_BUMP_EP[] : episode)

        # aggiorna best mediana10
        med10 = Statistics.median(episode_fidelities[max(1, end-9):end])
        BEST_MED10[] = max(BEST_MED10[], med10)

        if debug
            @info("↑ Threshold: $(round(old_thr; digits=3)) → $(round(SUCCESS_THR[]; digits=3))",
                  sr_win=round(sr_win; digits=2),
                  sr_streak=round(sr_streak; digits=2),
                  medF=round(medF; digits=3),
                  qf=round(qf; digits=3),
                  gate=round(q_gate; digits=3))
        end
    ...
    # ---- LOG periodico se niente bump ↑ ----
    if debug && episode % 5 == 0
        @info("No bump",
              sr_win=round(sr_win; digits=2),
              sr_streak=round(sr_streak; digits=2),
              medianF=round(medF; digits=3),
              qf=round(qf; digits=3),
              target=round(dyn_target; digits=2),
              gate=round(q_gate; digits=3))
    end

    # ---- ROLLBACK / BUMP ↓ ----
    if allow_down_bump && THR_IDX[] > 1 && episode > 0
        # mediana ultimi 10 episodi (o meno se m<10)
        med10 = Statistics.median(episode_fidelities[max(1, end-9):end])

        # aggiorna BEST_MED10 se stai migliorando
        if med10 > BEST_MED10[] + 1e-8
            BEST_MED10[] = med10
        end

        # condizioni per rollback:
        # 1) crollo della mediana10 rispetto al best
        # 2) finestra attuale molto sotto la soglia corrente
        # 3) SR medio finestra quasi nullo
        # 4) rispetto del cooldown per down-bump
        strong_drop   = (med10 < BEST_MED10[] - rollback_drop)
        far_below_thr = (medF < curr_thr - min_gap_for_down)
        sr_dead       = (sr_win < 0.15)
        can_down      = (episode - LAST_DOWN_EP[] ≥ down_cooldown)

        if strong_drop && far_below_thr && sr_dead && can_down
            old_thr = curr_thr
            THR_IDX[] = max(1, THR_IDX[] - 1)
            # isteresi inversa: spostati leggermente *sotto* la soglia per evitare ping-pong
            SUCCESS_THR[] = max(THR_LADDER[THR_IDX[]] - hysteresis_margin, 0.0)

            # al down-bump aumenta leggermente l'entropia e resetta gli optimizer
            agent.entropy_loss_weight = max(agent.entropy_loss_weight * 1.2, 0.02)
            try
                reset_opt_states!(agent)
            catch
                # se non disponibile, ignora
            end

            LAST_DOWN_EP[] = episode
            LAST_BUMP_EP[] = episode  # evita bump immediati nel prossimo paio di episodi

            if debug
                @warn("↓ Rollback threshold: $(round(old_thr; digits=3)) → $(round(SUCCESS_THR[]; digits=3))",
                      med10=round(med10; digits=3),
                      best_med10=round(BEST_MED10[]; digits=3),
                      sr_win=round(sr_win; digits=2))
            end
        end
    end
